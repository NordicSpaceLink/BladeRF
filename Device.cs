...
public sealed class Device : IDisposable
{
    ...
    {
        internal DevInfo devInfo;

        internal DeviceInfo(DevInfo info)
        {
            this.devInfo = info;
    ...
    {
        var ret = NativeMethods.CheckError(NativeMethods.get_device_list(out var devices));

        return Enumerable.Range(0, ret).Select(idx => new DeviceInfo(devices[idx])).ToList();
    }
    ...
    public Device(DeviceInfo info)
    {
        ...
        NativeMethods.CheckError(NativeMethods.open_with_devinfo(out dev, ref info.devInfo));
